...
export default function Rentals() {
  ...
  return <section className="space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-2xl font-bold">Locações</h1><p className="text-slate-600">Cotações, checklist, assinatura e financeiro.</p></div>{canCreate && <button className="btn-primary w-full sm:w-auto" onClick={openWizard}>Nova locação</button>}</div>
    ...
    {formOpen && <Modal title="Assistente de nova locação" onClose={() => setFormOpen(false)}>
      <form className="space-y-5" onSubmit={form.handleSubmit((values) => save.mutate(values))}>
        ...
        {step === 3 && <div className="space-y-4">
          ...
          <div className="mobile-actions flex flex-wrap gap-2"><button type="button" className="btn-secondary" onClick={() => setStep(2)}>Voltar</button><button className="btn-primary" disabled={save.isPending || !signatureOpen}>{save.isPending ? 'Salvando…' : 'Concluir locação e gerar termo'}</button></div>
        </div>}
      </form>
    </Modal>}
