...
export function RentalFinancialPanel({ rentalId }: { rentalId: string }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="card space-y-4">
      ...
      {canManage && <div className="flex flex-wrap gap-2"><button className="btn-secondary" type="button" disabled={document.isPending || paidTotal <= 0} onClick={() => document.mutate('RECEIPT')}>Emitir recibo de pagamento</button><button className="btn-secondary" type="button" disabled={document.isPending || extraTotal <= 0} onClick={() => document.mutate('EXTRA_RECEIPT')}>Emitir recibo de cobranças extras</button></div>}
      {error && <p className="text-sm text-red-700">{error instanceof Error ? error.message : 'Não foi possível concluir.'}</p>}
      <div className="grid gap-2">{visibleDocuments.map((item) => <div className="flex min-h-14 items-center justify-between gap-3 rounded-lg border border-slate-200 px-4" key={item.id}><div><p className="font-medium">{documentLabels[item.type] ?? item.type}</p><p className="text-xs text-slate-500">Atualizado em {new Date(item.created_at).toLocaleDateString('pt-BR')}</p></div><button type="button" className="btn-secondary" onClick={() => downloadDocument(item)}>Baixar</button></div>)}{visibleDocuments.length === 0 && <p className="text-sm text-slate-500">Nenhum documento gerado.</p>}</div>
    </div>
